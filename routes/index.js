...
var db = new sqlite3.Database('/home/pi/workspace/RaspberryServer/raspdb.db');
                            ...
                            db.each("SELECT * FROM latitude WHERE id = (SELECT MAX(id) FROM latitude);", function(err, row) {
                                ...
                                db.each("SELECT * FROM longitude WHERE id = (SELECT MAX(id) FROM longitude);", function(err, row) {
                                    ...
                                    console.log(row.id + ": " + humiditydata + ' ' + temporarydata + ' ' + pmdata + ' ' +
                                        lightdata + ' ' + timestamp + ' ' + longtitude + ' ' + latitude);
...
module.exports = function(app) {
    app.get('/', function(req, res) {
        request.get('http://45.63.50.188/login').timeout(3000).end(function(err, result) {
            if (err) {
                console.log(err);
